...
`orbit-sec-oauth-index` is centered on security tooling work in PHP. The technical target is: Implement a PHP security tooling project for oauth incremental indexing, using append-only fixtures and checkpoint recovery checks.
...
The score is deterministic. The current threshold is 171. Risk has a penalty of 5, latency has a penalty of 3, and weight contributes a bonus of 2.
